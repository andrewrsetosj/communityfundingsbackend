...
async def dashboard(admin_id: int, db: AsyncSession = Depends(get_db)):
    ...
    stats = await db.execute(text("""
        SELECT
            ...
            (SELECT COUNT(*) FROM (
            SELECT
                cr.report_id::TEXT          AS report_id,
                ...
                cr.notes                    AS details,
                cr.status                   AS status,
                cr.time_reported            AS time_created
            FROM campaign_reports cr
            UNION ALL
            SELECT
                cmr.report_id::TEXT, 'comment'::TEXT,
                cmr.reporter_creator_id,
                cmr.reported_campaign_id,
                cmr.reported_comment_id,
                cmr.reported_comment_creator_id,
                cmr.reason, cmr.notes, cmr.status, cmr.time_reported
    ...
    alerts = await db.execute(text("""
        SELECT
            ...
            SELECT
                ...
                mr.status, mr.time_created
            FROM misc_reports mr
        ) WHERE time_created >= NOW() - INTERVAL '24 hours') AS new_reports,
    ...
    r = await db.execute(text(f"""
        SELECT c.campaign_id, c.title, c.status, c.creator_id, cr.name AS creator_name,
            ...
            SELECT
                ...
                mr.status, mr.time_created
            FROM misc_reports mr
        ) r WHERE CAST(r.campaign_id AS TEXT) = CAST(c.campaign_id AS TEXT)) AS report_count
        FROM campaigns c
        LEFT JOIN creators cr ON cr.creator_id = c.creator_id
        {where}
    ...
    try:
        rc = await db.execute(text(f"""
            ...
            SELECT
                ...
                mr.status, mr.time_created
            FROM misc_reports mr
        ) r ON CAST(r.campaign_id AS TEXT) = CAST(c.campaign_id AS TEXT)
            LEFT JOIN creators cr ON cr.creator_id = c.creator_id
