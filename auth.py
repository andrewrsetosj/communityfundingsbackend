...
#     # Try to find local user by clerk_id first (if token was RS256)
#     result = await db.execute(select(User).where(User.clerk_id == user_id))
#     user = result.scalar_one_or_none()

#     # If not a Clerk-linked user, fallback to the previous behavior (search by id)
#     if not user:
#         result = await db.execute(select(User).where(User.id == user_id))
#         user = result.scalar_one_or_none()

#     # Optional: if still not found and you know this is a Clerk token, create user
#     # (Only do this if you're OK auto-provisioning.)
#     if not user and is_probably_clerk_token:
#         user = User(clerk_id=user_id, email=None, name=None)  # fill fields as you wish
#         db.add(user)
#         await db.flush()